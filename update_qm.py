...
with open('src/logic/quineMcCluskey.ts', 'r') as f:
    content = f.read()

lines = content.split('\n')
idx = -1
for i, line in enumerate(lines):
    if line.startswith('export function quineMcCluskey'):
        idx = i
        break

if idx != -1:
    new_func = """export function quineMcCluskey(minterms: number[], numVars: number = 4): { result: string, steps: {title: string, details: string}[] } {
  ...
  return { result, steps };
    ...
    with open('src/logic/quineMcCluskey.ts', 'w') as f:
        f.write(before + '\n' + new_func + '\n')
