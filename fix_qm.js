...
const content = fs.readFileSync('src/logic/quineMcCluskey.ts', 'utf8');
const lines = content.split('\n');

const startIndex = lines.findIndex(l => l.startsWith('export function quineMcCluskey'));
if (startIndex !== -1) {
  const newFunc = `export function quineMcCluskey(minterms: number[], numVars: number = 4): { result: string, steps: {title: string, details: string}[] } {
  ...
  fs.writeFileSync('src/logic/quineMcCluskey.ts', before + '\n' + newFunc);
}
