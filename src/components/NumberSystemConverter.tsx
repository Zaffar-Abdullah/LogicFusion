            ...
            {isValid ? (
              <div className="space-y-3 font-mono text-xs text-slate-600 dark:text-slate-400 max-h-64 overflow-y-auto custom-scrollbar pr-2">
                <div className="p-3 bg-white dark:bg-slate-950 rounded border border-slate-100 dark:border-slate-800 shadow-sm">
                  <div className="font-semibold text-indigo-600 dark:text-indigo-400 mb-1">1. Parse Input to Decimal</div>
                  <div>Input: <span className="text-slate-800 dark:text-slate-200">{inputValue}</span> (Base {inputBase})</div>
                  <div>Parsed Value = <span className="font-bold text-slate-800 dark:text-slate-200">{decimalValue}</span></div>
                </div>
                
                {outputBase !== 10 && (
                  <div className="p-3 bg-white dark:bg-slate-950 rounded border border-slate-100 dark:border-slate-800 shadow-sm">
                    <div className="font-semibold text-indigo-600 dark:text-indigo-400 mb-2">2. Repeated Division by {outputBase}</div>
                    <table className="w-full text-left">
                      <thead>
                        <tr className="border-b border-slate-200 dark:border-slate-800">
                          <th className="py-1">Value</th>
                          <th className="py-1">÷ {outputBase}</th>
                          <th className="py-1 text-right">Remainder</th>
                        </tr>
                      </thead>
                      <tbody>
                        {(() => {
                          ...
                          if (curr === 0) {
                            steps.push(
                              <tr key={0}>
                                <td>0</td><td>0</td><td className="text-right text-indigo-500 font-bold">0</td>
                              </tr>
                            );
                          } else {
                            let k = 0;
                            while (curr > 0) {
                              ...
                              steps.push(
                                <tr key={k++} className="border-b border-slate-100 dark:border-slate-800/50 last:border-0">
                                  <td className="py-1">{curr}</td>
                                  <td className="py-1">{q}</td>
                                  <td className="py-1 text-right text-indigo-500 font-bold">{displayR}</td>
                                </tr>
                              );
                              curr = q;
                            }
                    ...
                    <div className="mt-2 text-[10px] text-slate-500 text-right">Read remainders from bottom to top</div>
                  </div>
                )}
                
                <div className="p-3 bg-white dark:bg-slate-950 rounded border border-slate-100 dark:border-slate-800 shadow-sm">
                  <div className="font-semibold text-indigo-600 dark:text-indigo-400 mb-1">Final Result</div>
                  <div className="text-sm text-slate-800 dark:text-slate-200 font-bold">{convertedOutput} <sub className="text-[10px] font-normal text-slate-500">({outputBase})</sub></div>
                </div>
              </div>
            ) : (
